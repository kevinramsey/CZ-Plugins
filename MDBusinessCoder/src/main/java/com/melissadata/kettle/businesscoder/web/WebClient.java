...
public class WebClient {
	 ...
	 */
	public int call(final URL url, final JSONObject jRequest, final StringBuffer response, final int attempts) throws KettleException {
		...
		try {
			if ((log != null) && log.isDetailed()) {
				log.logDetailed(MDBusinessCoderStep.getLogString("ConnectingToURL", url.toString(), "" + myRetries));
			}
			...
			FutureTask<Integer> task = new FutureTask<Integer>(new Callable<Integer>() {
				@Override public Integer call() throws Exception {
					return callInThreadPool(url, jRequest, response, attempts);
				}
			});
		...
		if ((log != null) && log.isError()) {
			log.logError(MDBusinessCoderStep.getErrorString("TimeoutDetected", timeoutMessage, url.toString()));
		}

		// We only get here if there were timeouts ;
		MDBusinessCoderWebService.setErrorMessage(timeoutMessage);
		return statusCode;
	}
	 ...
	 */
	private int callInThreadPool(URL url, JSONObject jRequest, StringBuffer response, int attempts) throws HttpException, IOException {
		...
		if (jRequest != null) {
			...
			if ((log != null) && log.isRowLevel()) {
				log.logRowlevel("JSON Request: " + jRequest.toJSONString());
			}

			method = new PostMethod(url.toString());
			((PostMethod) method).setRequestEntity(new StringRequestEntity(jRequest.toJSONString(), "application/json", "UTF-8"));
		} else {
			// Prepare HTTP GET
		...
		try {
			...
			if ((log != null) && log.isDebug()) {
				log.logDebug(MDBusinessCoderStep.getLogString("ResponseCode", String.valueOf(statusCode)));
			}
			if (statusCode != -1) {
				if (Const.isEmpty(webEncoding)) {
					inputStreamReader = new InputStreamReader(method.getResponseBodyAsStream());
					if ((log != null) && log.isDebug()) {
						log.logDebug(MDBusinessCoderStep.getLogString("Encoding not set using: ", inputStreamReader.getEncoding()));
					}
				} else {
					inputStreamReader = new InputStreamReader(method.getResponseBodyAsStream(), webEncoding);
					if ((log != null) && log.isDebug()) {
						log.logDebug(MDBusinessCoderStep.getLogString("Encoding", inputStreamReader.getEncoding()));
					}
				}
				...
				if ((log != null) && log.isRowLevel()) {
					log.logRowlevel(MDBusinessCoderStep.getLogString("ResponseBody", response.toString()));
				}
			}
			return statusCode;
	...
	public int checkProxy(final URL url, final StringBuffer request, final StringBuffer response) {
// FIXME do real proxy check disabled for now
// int statCode = 200;
// proxyCheck = true;
// try {
// statCode = call(url, request, response, 0);
// } catch (KettleException e) {
// MDBusinessCoderData.webInitMsg = e.getMessage();
// }
// proxyCheck = false;
// return statCode;
		return 200;
