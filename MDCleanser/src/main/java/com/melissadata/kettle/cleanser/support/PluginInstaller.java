...
public class PluginInstaller {
	...
	public class dataPathDialog extends MDAbstractDialog {
		...
		protected void createContents(Composite parent, Object arg1) {
			helper = new MDCleanserHelper((MDCleanserDialog) dialog);
			//	helper = par
		...
		protected String getStringPrefix() {
			return "MDCleanser.PluginInStaller";
		}
	...
	private static       Class<?>            PKG                  = PluginInstaller.class;
	...
	public PluginInstaller(boolean isContactZone, LogChannelInterface log) {
		this.log = log;
		if (isContactZone) {
			log.logBasic("ContactZone MDCleanser Installed");
		} else {
			log.logBasic("Pentaho Install MDCleanser ");
			getSrcDir();
			setDataDirDefault();
		}
	}
	...
	private void updateCZ() {
		Props.getInstance().setProperty(TAG_CLEANSER_VERSION, version);
	...
	public void doInstall(boolean isInstalled, boolean isUpdate, String currentVersion) throws KettleException {
		...
		if (isUpdate) {
			log.logBasic("Updating MDGeneralizedCleanser Plugin to  " + version);
		} else {
			log.logBasic("Installing MDGeneralizedCleanser Plugin " + version);
		}
		if (!installed) {
			...
			log.logBasic("Updating MD Generalized Cleanser");
			try {
				loadClassFiles();
			} catch (Exception e) {
				log.logError("Error loading class files: " + e.getMessage());
	...
	private void setDataDirDefault() {
		if (Const.isWindows()) {
			...
			dataDir = new File(Const.getUserHomeDirectory()+ fileSep + "MDdata");
