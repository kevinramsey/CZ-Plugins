...
public class PassThruFields {
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev, String[] input, String[] output, RowMetaInterface info) {
		...
		if ((prev != null) && (prev.size() > 0)) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.StepReceivingFields", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
			...
			if (error_found) {
				error_message = BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.FilterOutFieldsNotFound", "") + Const.CR + Const.CR + error_message; //$NON-NLS-1$
			...
			if (filterOut.size() > 0) {
				...
				if (error_found) {
					error_message = BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.FieldsNotFound", "") + Const.CR + Const.CR + error_message; //$NON-NLS-1$
			...
			if (error_found) {
				error_message = BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.PassThruFieldsNotFound", "") + Const.CR + Const.CR + error_message; //$NON-NLS-1$
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.AllFilterOutFieldsFound", ""), stepMeta); //$NON-NLS-1$
				remarks.add(cr);
			}
			if (passThru.size() > 0) {
				// Starting from prev...
			...
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "MDCleanserMeta.CheckResult.FieldsNotFound2", ""), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	 ...
	 * @return
	 */
	public boolean selectIndexes(MDCleanserStep step, MDCleanserData data, IOMetaHandler ioMeta) {
		...
		for (int i = 0; i < passThruFieldNrs.length; i++) {
			...
			if (passThruFieldNrs[i] < 0) {
				step.logError(BaseMessages.getString(PKG, "MDCleanser.Log.CouldNotFindField", passThru.get(i))); //$NON-NLS-1$
				step.setErrors(1);
				step.stopAll();
				return false;
			}
