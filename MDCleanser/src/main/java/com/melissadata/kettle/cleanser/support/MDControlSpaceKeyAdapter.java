...
public class MDControlSpaceKeyAdapter extends KeyAdapter {
	...
	private static final String		CLEANSER_RESULT_CODE	= "MDCleanser.ResultCode.";

	private static Class<?>			PKG							= MDControlSpaceKeyAdapter.class;
...
// toolTip.setImage(GUIResource.getInstance().getImageSpoon());
	...
	private String getResultCodeDetail(String rc) {
		...
		String dtl = CLEANSER_RESULT_CODE + rc + ".Detail";
		String description = BaseMessages.getString(PKG, dtl, "");
	...
	private String[] getResultCodes() {
		...
		if ((resultCodes == null) || (resultCodes.length < 1)) {
			...
			rcs.addAll(getResultCodes("Abbreviation", "KA"));	// Abbreviation
			rcs.addAll(getResultCodes("Casing", "KC")); 	// Casing
			rcs.addAll(getResultCodes("TransformExpression", "KE")); 	// Transform Expression
			rcs.addAll(getResultCodes("Punctuation", "KP")); 	// Punctuation
			rcs.addAll(getResultCodes("RegEx", "KR")); 	// RegEx
			rcs.addAll(getResultCodes("SearchReplace", "KS")); 	// Search & Replace
	...
	//	String tst = "com.melissadata.kettle.businesscoder";
		...
		if (bundle != null) {
			...
			while (keys.hasMoreElements()) {
				...
				String categoryPrefix = CLEANSER_RESULT_CODE + category + ".";
