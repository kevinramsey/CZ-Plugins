...
import com.melissadata.kettle.evaluator.Casts.CastRec;
import com.melissadata.kettle.evaluator.EvalItem;
import com.melissadata.kettle.evaluator.EvalItem.eDataType;
import com.melissadata.kettle.evaluator.EvaluatorException;
import com.melissadata.kettle.evaluator.Functions.FunctionRec;
import com.melissadata.kettle.evaluator.Operators;
import com.melissadata.kettle.evaluator.Operators.OperatorRec;
...
/* Function           */ {eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.MoveAndRetry,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp  ,eStackOperations.PushToTemp,eStackOperations.MoveAndDelete ,eStackOperations.IgnoreToken ,eStackOperations.PushToTemp}};
	...
	public Stack<EvalItem> stringToStack(String cleanExp) throws EvaluatorException {
		...
		for (int i = 0; i < cleanExp.length();) {
			...
			while (!stacked) {
				//System.out.println(" NEW ITEM = " + newItem.value + " - Stacked:" + stacked);
				...
				if (newItem.type == EvalItem.eTypes.Column || newItem.type == EvalItem.eTypes.Constant || newItem.type == EvalItem.eTypes.Variable) {
					if(newItem.value == null) { // we need a value for testing. Because in testing we don't get column or variable
												// values we supply a "HOLDER" value here.
						newItem.value = "HOLDER";
					}
	...
	private eTokenTypes ParseToken(String expression, boolean unaryOk,
		...
		//System.out.println("Parse Token : " + expression + "  peek = " + peek);
	...
	private static OperatorRec[] OperatorList = new OperatorRec[] {
		new OperatorRec("||",EvalItem.eOperators.LogicalOr, Operators.EvalLogicalOr, Operators.TestLogicalOr,2),new OperatorRec("&&",EvalItem.eOperators.LogicalAnd,Operators.EvalLogicalAnd,Operators.TestLogicalAnd,2),
		new OperatorRec("==",EvalItem.eOperators.Equal,Operators.EvalEqual,Operators.TestEqual,2),new OperatorRec("!=",EvalItem.eOperators.NotEqual,Operators.EvalNotEqual,Operators.TestNotEqual,2),
		new OperatorRec(">=",EvalItem.eOperators.GreaterEqual,Operators.EvalGreaterEqual,Operators.TestGreaterEqual,2),new OperatorRec("<=",EvalItem.eOperators.LessEqual,Operators.EvalLessEqual,Operators.TestLessEqual, 2),
		new OperatorRec("?",EvalItem.eOperators.Ternary,Operators.EvalTernary1,Operators.TestTernary1,1),new OperatorRec(":",EvalItem.eOperators.Ternary,Operators.EvalTernary3,Operators.TestTernary3,3),
		new OperatorRec("!",EvalItem.eOperators.LogicalNot,Operators.EvalLogicalNot,Operators.TestLogicalNot,1),/*new OperatorRec("|",EvalItem.eOperators.BitwiseOr,EvalBitwiseOr,TestBitwiseOr,2),
		...
		new OperatorRec(">",EvalItem.eOperators.Greater,Operators.EvalGreater,Operators.TestGreater,2),new OperatorRec("<",EvalItem.eOperators.Less,Operators.EvalLess,Operators.TestLess,2),
		new OperatorRec("+",EvalItem.eOperators.Plus,Operators.EvalPlus,Operators.TestPlus,2),new OperatorRec("-",EvalItem.eOperators.Minus,Operators.EvalMinus,Operators.TestMinus,2),
		new OperatorRec("*",EvalItem.eOperators.Multiply,Operators.EvalMultiply,Operators.TestMultiply,2),new OperatorRec("/",EvalItem.eOperators.Divide,Operators.EvalDivide,Operators.TestDivide,2),
		new OperatorRec("%",EvalItem.eOperators.Modulus,Operators.EvalModulus,Operators.TestModulus,2),/*new OperatorRec("~",EvalItem.eOperators.BitwiseNot,EvalBitwiseNot,TestBitwiseNot,2),*/
		new OperatorRec("(",EvalItem.eOperators.LeftParenthesis,null,null,0),new OperatorRec(")",EvalItem.eOperators.RightParenthesis,null,null,2),
		new OperatorRec(",",EvalItem.eOperators.Comma,null,null,0)};
	...
	private static FunctionRec[] FunctionList = new FunctionRec[] { new FunctionRec("ABS(", Functions.EvalAbs, Functions.TestAbs, 1),
			...
			new FunctionRec("ISIBAN(",Functions.EvalIsIBAN, Functions.TestIsIBAN,1),
	...
	// Casts - 
	private static CastRec[] CastList = new CastRec[] {

	new CastRec("(DT_DECIMAL", Casts.EvalCastDT_DECIMAL, Casts.TestCastDT_DECIMAL, 1), new CastRec("(DT_NUMERIC", Casts.EvalCastDT_NUMERIC, Casts.TestCastDT_NUMERIC, 1),
			new CastRec("(DT_BIGNUMBER", Casts.EvalCastDT_BIGNUMBER, Casts.TestCastDT_BIGNUMBER, 1), new CastRec("(DT_STR", Casts.EvalCastDT_STR, Casts.TestCastDT_STR, 1),
			new CastRec("(DT_NULL", Casts.EvalCastDT_NULL, Casts.TestCastDT_NULL, 1), new CastRec("(DT_DATE", Casts.EvalCastDT_DATE, Casts.TestCastDT_DATE, 1),
			new CastRec("(DT_TIMESTAMP", Casts.EvalCastDT_TIMESTAMP, Casts.TestCastDT_TIMESTAMP, 1), new CastRec("(DT_INETADDRESS", Casts.EvalCastDT_INETADDRESS, Casts.TestCastDT_INETADDRESS, 1),
			new CastRec("(DT_BOOL", Casts.EvalCastDT_BOOL, Casts.TestCastDT_BOOL, 1) };
	...
	private int ParseCast(String str, boolean peek) throws EvaluatorException {
		...
		for (int i = 0; i < CastList.length; i++) {
			...
			if (str.length() >= CastList[i].CastType.length() && CastList[i].CastType.equalsIgnoreCase(str.substring(0, CastList[i].CastType.length()))) {
				...
				if (CastList[i].CastType.equals("(DT_DATE")) {
				...
				} else if(CastList[i].CastType.equals("(DT_DECIMAL")) {
					...
					String[] paramList = str.substring(1, str.indexOf(')')/* - 1 */).split(",");
					if (paramList.length != 2)
						throw new EvaluatorException("Syntax Error " + CastList[i].CastType + " missing scale");
				...
				}  else if(CastList[i].CastType.equals("(DT_NUMERIC")) {
					...
					if (paramList.length != 1)
						throw new EvaluatorException("Syntax Error " + CastList[i].CastType + " to many parameters");

					if (peek)
						peekItem = new EvalItem(CastList[i].Function, CastList[i].TestFunction, CastList[i].CastType + ")", CastList[i].ParameterCount, ""/*paramList[1]*/, 0 /*Integer.parseInt(paramList[2])*/);
					else
						newItem = new EvalItem(CastList[i].Function, CastList[i].TestFunction, CastList[i].CastType + ")", CastList[i].ParameterCount, ""/*paramList[1]*/ , 0 /*Integer.parseInt(paramList[2])*/);

					return CastList[i].CastType.length() /* + paramList[1].length() + paramList[2].length()*/ + 1;

				}  else if(CastList[i].CastType.equals("(DT_STR")) {
					...
					if (paramList.length != 3)
						throw new EvaluatorException("Syntax Error " + CastList[i].CastType + " missing scale or codepage");
					...
					return CastList[i].CastType.length() + paramList[1].length() + paramList[2].length() + 3/*account for commas*/;
		...
		for (int c = 0; c < varNames.length; c++) {
			...
			if (varNames[c] != null) {
				if (str.length() >= varNames[c].length() && str.substring(0, varNames[c].length()).equals(varNames[c])) {
					...
					if (peek)
						peekItem = new EvalItem(EvalItem.eTypes.Variable, varNames[c], eDataType.DT_STR);
					else
						newItem = new EvalItem(EvalItem.eTypes.Variable, varNames[c], eDataType.DT_STR);
