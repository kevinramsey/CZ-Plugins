...
/**
 *  MDBinaryEvaluator class is the work horse for evaluating Melissadata result code expressions.
 * <p>
...
/*)*/   { eStackOperations.Error, eStackOperations.Error, eStackOperations.Error, eStackOperations.Error, eStackOperations.Error } };
	...
	/**
	 * @param expression The Melissadata result code expression used to initialize the  MDBinaryEvaluator class object
	 *
	 * @throws KettleException If error is found in the expression
	 */
	...
	private void stringToStack(String expression) throws KettleException {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			if (thisToken == eTokenType.ResultCode && (lastToken.isOneOf(eTokenType.ResultCode, eTokenType.RightParenthesis) || nextToken.isOneOf(eTokenType.ResultCode, eTokenType.LeftParenthesis)))
			...
			else if (thisToken.isOneOf(eTokenType.And, eTokenType.Or) && (lastToken.isOneOf(eTokenType.BeginningOfExpression, eTokenType.And, eTokenType.Or, eTokenType.Not, eTokenType.LeftParenthesis) || nextToken
					.isOneOf(eTokenType.EndOfExpression, eTokenType.And, eTokenType.Or, eTokenType.RightParenthesis)))
				throw new EvaluatorException("SyntaxErrorInExpression", expression);
				...
				throw new EvaluatorException("SyntaxErrorInExpression", expression);
			else if (thisToken == eTokenType.RightParenthesis && (lastToken.isOneOf(eTokenType.BeginningOfExpression, eTokenType.And, eTokenType.Or, eTokenType.Not, eTokenType.LeftParenthesis) || nextToken
					.isOneOf(eTokenType.ResultCode, eTokenType.Not, eTokenType.LeftParenthesis)))
				throw new EvaluatorException("SyntaxErrorInExpression", expression);
	 ...
	 *
	 * @param tokens Array of strings to tokenize.
	 * @param index  The index of the string to tokenize
	 * @param resultCode Holder for resultCode strings
	 * @return eTokenType  The token type of the tokenized string.
	 */
	private eTokenType tokenType(String[] tokens, int index, String[] resultCode) {
		...
		if (isResultCode(/*fields_, */token, resultCode))
			return eTokenType.ResultCode;
		else if (isWildcard(/*fields_, */token, resultCode))
	 ...
	 * @param resultCode Holder for the ResultCodes.
	 * @return true if token is a resulrCode
	 */
	...
	/**
	 * Called to determine if a token is a wild card result code string
	 *
	 * @param token  Token to evaluate.
	 * @param resultCode Holder for the ResultCodes.
	 * @return true if token is wildcard
	 */
	...
	/**
	 * Called to evaluate the result codes and determine if they pass according to
	 * the current validation expression
	 *
	 * @param resultCodes List of result codes to be evaluated.
	 * @return True if result codes pass evaluation.
	 * @throws EvaluatorException If there is an error with the expression.
	 */
	...
	/**
	 * Enumerations defining the various token types:
	 */
	public enum eTokenType {
		BeginningOfExpression, EndOfExpression, ResultCode, WildCardField, And, Or, Not, LeftParenthesis, RightParenthesis, InvalidToken,;
	...
	public static class EvaluatorException extends KettleException {
		...
		private static String getMessage(String messageID, Object... args) {
			return BaseMessages.getString(PKG, "MDSupport.BinaryEvaluator.Error." + messageID, args);
		}
	}

	/**
	 * Class representing individual Evaluation Items.
	 */
	public static class EvalItem {
		...
		public static enum eType {
			ResultCode, WildResultCode, Operator, Value
		}

		public static enum eOperator {
			And, Or, Not, LeftParenthesis, RightParenthesis
		}
	}
}
