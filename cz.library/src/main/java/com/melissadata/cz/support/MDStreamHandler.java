...
/**
 * Class to manage the target streams defined by the user.
 */
...
public class MDStreamHandler {
	...
	private static StreamInterface newFilterTargetStream = new Stream(StreamType.TARGET, null, BaseMessages.getString(PKG,
			"MDSupport.TargetStream.NewFilterTarget.Description"), StreamIcon.TARGET, null);
	...
	public static void getStepIOMeta(StepIOMetaInterface ioMeta, List<FilterTarget> filterTargets) {
		...
		for (FilterTarget target : filterTargets) {
			if (!Const.isEmpty(target.getName())) {
				String description = BaseMessages.getString(PKG, "MDSupport.TargetStream.FilterTarget.Description", Const.NVL(target.getName(), ""));
