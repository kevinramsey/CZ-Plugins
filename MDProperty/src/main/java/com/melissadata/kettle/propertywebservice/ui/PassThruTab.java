...
import com.melissadata.cz.support.MDTransfer;

public class PassThruTab implements MDTab {

	private static Class<?>			PKG		= MDPropertyWebServiceMeta.class;

	protected static final int[]	EMPTY	= new int[0];

	protected MDPropertyWebServiceDialog	dialog;
	...
	public PassThruTab(MDPropertyWebServiceDialog dialog, TransMeta transMeta) {
	...
	public void getData(BaseStepMeta meta) {
		...
		List<String> passThruNames = bcMeta.passThruFields.passFilterFields.get(PassThruFields.TAG_PASS_THROUGH);
		...
		bcMeta.passThruFields.passFilterFields.put(PassThruFields.TAG_PASS_THROUGH, passThruNames);

		// Get filtered fields
		List<String> filterOutNames = bcMeta.passThruFields.passFilterFields.get(1);
		...
		bcMeta.passThruFields.passFilterFields.put(PassThruFields.TAG_FILTER_OUT, filterOutNames);
	}
	...
	public String getHelpURLKey() {
		return BaseMessages.getString(PKG,"MDPropertyWebService.Plugin.Help.PassThruTab");
	}
	...
	public boolean init(BaseStepMeta meta) {
		...
		List<String> passThruNames = bcMeta.passThruFields.passFilterFields.get(PassThruFields.TAG_PASS_THROUGH);
		List<String> filterOutNames = bcMeta.passThruFields.passFilterFields.get(PassThruFields.TAG_FILTER_OUT);
	...
	private void addDND(final TableViewer viewer, final String id) {
		...
		final MDTransfer myTransfer = MDTransfer.getInstance();
...
// System.out.println("dragSetData.1:");
					...
					MDTransfer.Type datum = new MDTransfer.Type();
					datum.setID(id);
...
// System.out.println("dragEnter.3:");
						...
						MDTransfer.Type datum = (MDTransfer.Type) myTransfer.nativeToJava(event.currentDataType);
						if (datum != null) {
...
// System.out.println("drop.1:");
					...
					int added = 0;
					MDTransfer.Type datum = (MDTransfer.Type) event.data;
					if (datum != null) {
...
// column.setMoveable(true);
	...
	protected String getString(String key, String... args) {
		...
		return BaseMessages.getString(PKG, "MDPropertyWebServiceDialog.PassThruTab." + key, args);
	}
