...
public class RequestManager {
	...
	public class Request {
		...
		private ArrayList<MDPropertyWebServiceRequest>	requests;
		private int										queue;
		private KettleException							exception;

		public Request(MDPropertyWebService service, List<MDPropertyWebServiceRequest> requests, int queue) {

			this.service 	= service;
			this.requests 	= new ArrayList<MDPropertyWebServiceRequest>(requests);
			this.queue = queue;
		}
		...
		public List<MDPropertyWebServiceRequest> getRequests() {
		...
		public void run() {
			int retries = service.serviceData.realWebRetries;
			int attempts = 0;
			...
			do {
				...
				try {
					...
					// Wrap the exception
					exception = new KettleException(MDPropertyWebServiceStep.getErrorString("RequestFailed"), t);
				}
				...
				if ((attempts > 0) && ((attempts % 10) == 0) && !stopped) {
					String message = "MD Property Web Service\n" + "We are having difficulties connecting to the service." + "\nWe will continue attempting to connect.\nCheck logs for additional information on the problem.";

					
					
					if (Thread.currentThread().getId() == ourGuyId) {
						showSleepMsg(message);
						service.log.logBasic("WARNING: MD Property Web Service -Is having trouble connecting to service :" + exception);
						service.log.logBasic("-- Continuing to retry conection to " + service.serviceData.realPropertyWebServiceURL + " Attempts: " + attempts);
				...
				if (attempts == service.serviceData.realWebRetries) {
					if (!fin) {
						fin = true;
						MDPropertyWebServiceStep.shMsg = true;
					}
...
//				// FIXME remove this
//				if(exception.getMessage().contains("Did not get result code")) {
//					exception = null;
//				}
//				retries = 0;
	...
	public void addRequest(MDPropertyWebService service, List<MDPropertyWebServiceRequest> requests, int queue) {
	...
	private void showSleepMsg(String message) {
		MDPropertyWebServiceStep.showSleepMsg(message);
