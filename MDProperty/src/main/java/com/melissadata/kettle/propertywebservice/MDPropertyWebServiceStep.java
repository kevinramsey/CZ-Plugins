...
// import com.melissadata.kettle.propertywebservice.IRequestManager;
...
public class MDPropertyWebServiceStep extends BaseStep implements StepInterface {

	public static String getErrorString(String string) {

		return BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.Error." + string);
	}

	public static String getErrorString(String string, String string2) {

		return BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.Error." + string, string2);
	}

	public static String getErrorString(String string, String timeoutMessage, String string2) {

		return BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.Error." + string, timeoutMessage, string2);
	}
	...
	public static String getFieldString(RowMetaInterface inputMeta, Object[] inputData, String field) throws KettleValueException {
		if (!Const.isEmpty(field)) { return inputMeta.getString(inputData, field, ""); }
		return null;
	}

	public static String getLogString(String string, String valueOf) {

		return BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.Log." + string, valueOf);
	}

	public static String getLogString(String key, String... args) {

		return BaseMessages.getString(PKG, "MDPropertyWebService.Meta.Log." + key, args);
	}

	public static String getLogString(String string, String string2, String string3) {

		return BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.Log." + string, string2, string3);
	}
	...
	public static Set<String> getResultCodes(String value) {
		...
		if ((value == null) || (value.trim().length() == 0)) { return new HashSet<String>(); }
	...
	public static void showSleepMsg(String message) {
		...
		if (isSpoon()) {
			if (shMsg) {
				shMsg = false;
				MessageBoxThread mbt = new MessageBoxThread(Spoon.getInstance().getShell(), "sleep", "Property Service", message);
	...
	private MDPropertyWebService			service;
	...
	public MDPropertyWebServiceStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans) {
	 ...
	 * @param requests
	 */
	public void addResultCodes(List<MDPropertyWebServiceRequest> requests) {
		...
		for (int recordID = 0; recordID < requests.size(); recordID++) {
			...
			if (!Const.isEmpty(bcMeta.propertyWebServiceFields.outputFields.get(PropertyWebServiceFields.TAG_OUTPUT_RESULTS).metaValue)) {
	...
	public synchronized boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		logDebug("MDProperty.init(smi=" + smi + ", sdi=" + sdi);
		shMsg = true;
		// Call parent handler first
		if (!super.init(smi, sdi)) { return false; }
		// Dereference the control structures
		MDPropertyWebServiceMeta meta = (MDPropertyWebServiceMeta) smi.clone();
		MDPropertyWebServiceData data = (MDPropertyWebServiceData) sdi;
		if(!MDPropertyWebServiceMeta.isEnterprise()){
			String message = BaseMessages.getString(PKG,"MDPropertyWebServiceDialog.Community.PopupMessage");
			if(isSpoon()) {
				MessageBoxThread mbt = new MessageBoxThread(Spoon.getInstance().getShell(), "community", "Property Web Service", message);
				Display.getDefault().asyncExec(mbt);
			}
			logError(message);
			stopAll();
		...
		if ((service = MDPropertyWebService.create(data, meta, this, getLogChannel())) == null) { return false; }
		// Initialize it
		try {
			service.init();
		...
		if (!service.checkProxy()) {
			logError(MDPropertyWebServiceData.webInitMsg);
			return false;
		}
		return true;
	}
	 ...
	 * @throws KettleValueException
	 */
	public boolean process(Object[] inputData, MDPropertyWebServiceData data, MDPropertyWebServiceMeta meta, IOMetaHandler ioMeta) throws KettleException {

		if (inputData != null) {
			// Build a request for this data
			MDPropertyWebServiceRequest request = service.buildRequest(ioMeta, inputData);
		...
		for (int queue = 0; queue < data.requests.size(); queue++) {
			...
			// then process them
			List<MDPropertyWebServiceRequest> requests = data.requests;
			if (((inputData == null) && (requests.size() > 0)) || (requests.size() >= data.maxRequests)) {
				...
				if (!handleCompletedRequests(meta, data, false)) { return false; }
			}
		}
		return true;
	}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		...
		bcMeta = (MDPropertyWebServiceMeta) smi;
		MDPropertyWebServiceData data = (MDPropertyWebServiceData) sdi;
	 ...
	 */
	private boolean handleCompletedRequests(MDPropertyWebServiceMeta meta, MDPropertyWebServiceData data, boolean done) throws KettleStepException {
		...
		while ((result = requestManager.getCompletedRequest(done)) != null) {
			...
			List<MDPropertyWebServiceRequest> requests = result.getRequests();
			try {
	 ...
	 * @throws KettleException
	 */
	protected boolean processComplete(MDPropertyWebServiceMeta meta, MDPropertyWebServiceData data) throws KettleException {

		// Wait for all outstanding requests to complete
		if (!handleCompletedRequests(meta, data, true)) { return false; }
		return true;
	}
	 ...
	 * @throws KettleException
	 */
	protected synchronized boolean processInit(MDPropertyWebServiceMeta meta, MDPropertyWebServiceData data) throws KettleException {
		...
		if (!meta.passThruFields.selectIndexes(this, data, sourceIO)) { return false; }
		...
		for (int i = 0; i < filterTargets.size(); i++) {
			...
			for (StreamInterface targetStream : targetStreams) {
				if ((filterTarget.getTargetStepname() != null) && filterTarget.getTargetStepname().equals(targetStream.getStepname())) {
					...
					if (targetRowSet == null) { throw new KettleException(getErrorString("TargetStepInvalid", targetStream.getStepname())); }
