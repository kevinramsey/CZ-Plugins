package com.melissadata.kettle.propertywebservice.support;
	...
	public class MDControlSpaceKeyAdapter extends KeyAdapter {
		...
		private static final String		PROPERTY_WEB_SERVICE_RESULT_CODE	= "MDPropertyWebServiceMeta.ResultCode.";
	...
	// toolTip.setImage(GUIResource.getInstance().getImageSpoon());
		...
		private String getResultCodeDetail(String rc) {
			...
			String description = BaseMessages.getString(PKG, PROPERTY_WEB_SERVICE_RESULT_CODE + rc + ".Detail");
		...
		private String[] getResultCodes() {
			...
			if ((resultCodes == null) || (resultCodes.length < 1)) {
				...
				rcs.addAll(getResultCodes("PropertyService", "YS"));
				rcs.addAll(getResultCodes("PropertyService", "YE"));
		...
		private Collection<String> getResultCodes(String category, String prefix) {
			...
			String tst = "com.melissadata.kettle.propertywebservice";
			...
			if (bundle != null) {
				...
				while (keys.hasMoreElements()) {
					...
					String categoryPrefix = PROPERTY_WEB_SERVICE_RESULT_CODE + category + ".";
