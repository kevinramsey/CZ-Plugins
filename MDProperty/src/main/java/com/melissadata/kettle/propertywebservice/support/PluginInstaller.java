...
public class PluginInstaller {
	...
	private void updateCZ() {
		...
		Props.getInstance().setProperty(TAG_PROPERTY_WEB_SERVICE_VERSION, version);
	...
	public void doInstall(boolean isInstalled, boolean isUpdate, String currentVersion) throws KettleException {
		...
		if (isUpdate) {
			log.logBasic("Updating MDProperty Plugin to  " + version);
		} else {
			log.logBasic("Installing MDProperty Plugin " + version);
		}
		if (!installed) {
