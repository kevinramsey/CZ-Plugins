...
public class PassThruFields {
	...
	public static final String				TAG_PROPERTY_WEB_SERVICE_FILTER	= "property_web_service_filter";
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev, String[] input, String[] output, RowMetaInterface info) {
		...
		if ((prev != null) && (prev.size() > 0)) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.StepReceivingFields", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
			...
			if (error_found) {
				error_message = BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.FilterOutFieldsNotFound") + Const.CR + Const.CR + error_message; //$NON-NLS-1$
			...
			if (filterOut.size() > 0) {
				...
				if (error_found) {
					error_message = BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.FieldsNotFound") + Const.CR + Const.CR + error_message; //$NON-NLS-1$
			...
			if (error_found) {
				error_message = BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.PassThruFieldsNotFound") + Const.CR + Const.CR + error_message; //$NON-NLS-1$

				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.AllFilterOutFieldsFound"), stepMeta); //$NON-NLS-1$
				remarks.add(cr);
			}
			...
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "MDPropertyWebServiceMeta.CheckResult.FieldsNotFound2"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	 ...
	 * @return
	 */
	public boolean selectIndexes(MDPropertyWebServiceStep step, MDPropertyWebServiceData data, IOMetaHandler ioMeta) {
		...
		for (int i = 0; i < passThruFieldNrs.length; i++) {
			...
			if (passThruFieldNrs[i] < 0) {
				step.logError(BaseMessages.getString(PKG, "MDPropertyWebService.Log.CouldNotFindField", passThru.get(i))); //$NON-NLS-1$
				step.setErrors(1);
				step.stopAll();
				return false;
			}
