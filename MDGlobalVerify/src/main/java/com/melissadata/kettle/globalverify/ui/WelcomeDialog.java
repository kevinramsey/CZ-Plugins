...
public class WelcomeDialog extends MDAbstractDialog {
	...
	public WelcomeDialog(MDGlobalDialog dialog, int flags) {
		super(dialog, flags);
	}

	@Override
	public boolean cancel() {
		MDGlobalDialog.continueToAdvancedConfig = false;
		dispose();
		return true;
	}
	...
	protected void createContents(Composite parent, Object arg1) {
		...
		logo.setImage(ImageUtil.getImage(display, this.getClass(), "com/melissadata/kettle/globalverify/images/MelissaData.gif"));
		helper.setLook(logo);
	...
	@Override
	public void ok() {
		MDGlobalDialog.licString = wLicense.getText();
		MDGlobalDialog.continueToAdvancedConfig = true;
		dispose();
	}
}
