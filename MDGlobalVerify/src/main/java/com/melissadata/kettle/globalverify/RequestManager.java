...
public class RequestManager implements IRequestManager {
	...
	public class Request {
		...
		private MDGlobalService            service;
		private ArrayList<MDGlobalRequest> requests;
		private int                        queue;
		private KettleException            exception;

		public Request(MDGlobalService service, List<MDGlobalRequest> requests, int queue) {

			this.service = service;
			this.requests = new ArrayList<MDGlobalRequest>(requests);
			this.queue = queue;
		}
		...
		public List<MDGlobalRequest> getRequests() {
		...
		public void run() {
			...
			do {
				...
				try {
					...
					// Wrap the exception
					exception = new KettleException(MDGlobalVerify.getErrorString("RequestFailed"), t);
				}
				if (exception != null) {
					if (ourGuyId == 0) {
				...
				if ((attempts > 0) && ((attempts % 10) == 0) && !stopped) {
					String message = getLogString("Warning.SleepMessage");
					if (Thread.currentThread().getId() == ourGuyId) {
						showSleepMsg(message);
						service.log.logBasic(getLogString("Warning.TroubleConnecting", exception.getCause().toString()));
						service.log.logBasic(getLogString("Warning.ContinueRetry", service.checkData.realWebGlobalAddressVerifierURL.toString(),String.valueOf(attempts)));
				...
				if (attempts == service.checkData.realWebRetries) {
					if (!fin) {
						fin = true;
						MDGlobalVerify.shMsg = true;
					}
					String message = getLogString("Warning.NumerousAttempts");
					showSleepMsg(message);
				}
	...
	/**
	 * Special worker used when there is only one worker in the pool. It will run the request in the same thread as the
	 * caller and will block until it completes.
	 */
	...
	@Override
	public void addRequest(MDGlobalService service, List<MDGlobalRequest> requests, int queue) {
	...
	private void showSleepMsg(String message) {
		...
		MDGlobalVerify.showSleepMsg(message);
	}
