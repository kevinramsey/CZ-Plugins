...
public class MDControlSpaceKeyAdapter extends KeyAdapter {
	private static final String		MDCHECK_RESULT_CODE	= "MDGlobalAddress.ResultCode.";
...
// toolTip.setImage(GUIResource.getInstance().getImageSpoon());
	...
	private String[] getResultCodes() {
		if (resultCodes == null) {
			...
			// Name result codes
			rcs.addAll(getResultCodes("Name", "NS"));
			rcs.addAll(getResultCodes("Name", "NE"));
			...
			rcs.addAll(getResultCodes("Phone", "PC"));
			// Email result codes
			rcs.addAll(getResultCodes("Email", "EE"));
			rcs.addAll(getResultCodes("Email", "ES"));
			// Geocode
			rcs.addAll(getResultCodes("Geo", "GS"));
