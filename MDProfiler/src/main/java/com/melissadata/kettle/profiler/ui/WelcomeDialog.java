...
public class WelcomeDialog extends MDAbstractDialog {
	...
	public WelcomeDialog(MDProfilerDialog dialog, int flags) {
		super(dialog, flags);
	}

	@Override
	public boolean cancel() {
		MDProfilerDialog.continueToAdvancedConfig = false;
		dispose();
		return true;
	}
	...
	protected void createContents(Composite parent, Object arg1) {
		...
		logo.setImage(ImageUtil.getImage(display, this.getClass(), "com/melissadata/kettle/profiler/images/MelissaData.gif"));
		helper.setLook(logo);
		...
		licBtn.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				String url = getString("Trial.URL.Label");
				BrowserDialog.displayURL(getShell(), dialog, url);
			}
		});
	...
	@Override
	public void ok() {
		MDProfilerDialog.licString = wLicense.getText();
		MDProfilerDialog.continueToAdvancedConfig = true;
		dispose();
	}
}
