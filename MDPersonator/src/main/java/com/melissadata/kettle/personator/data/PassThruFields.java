...
import com.melissadata.kettle.personator.IOMetaHandler;
...
public class PassThruFields {
	...
	public static final String TAG_GLOBAL_FILTER = "global_address_filter";
	 ...
	 * @return 
	 */
	public boolean selectIndexes(MDPersonator step, MDPersonatorData data, IOMetaHandler ioMeta) {
		...
		for (int i = 0; i < passThruFieldNrs.length; i++) {
			...
			if (passThruFieldNrs[i] < 0) {
				step.logError(BaseMessages.getString(PKG, "MDPersonator.Log.CouldNotFindField", passThru.get(i))); //$NON-NLS-1$ 
				step.setErrors(1);
				step.stopAll();
				return false;
			}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
					"MDCheckMeta.CheckResult.StepReceivingFields", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
			...
			if (error_found) {
				...
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
						"MDCheckMeta.CheckResult.AllFilterOutFieldsFound"), stepMeta); //$NON-NLS-1$
				remarks.add(cr);
			}
			...
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG,
					"MDCheckMeta.CheckResult.FieldsNotFound2"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
