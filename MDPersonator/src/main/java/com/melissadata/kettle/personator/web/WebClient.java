...
public class WebClient {
	...
	public int call(final URL url, final String request, final StringBuffer response, final int attempts) throws KettleException {
		...
		try {
			if (log != null && log.isDetailed())
				log.logDetailed(MDPersonator.getLogString("ConnectingToURL", url.toString(), "" + myRetries));
		...
		if (log != null && log.isError())
			log.logError(MDPersonator.getErrorString("TimeoutDetected", timeoutMessage, url.toString()));

		// We only get here if there were timeouts ;
		PersonatorWebService.setErrorMessage(timeoutMessage);
		return statusCode;
	}
	...
	private int callInThreadPool(URL url, String request, StringBuffer response, int attempts) throws HttpException, IOException {
		...
		try {
			...
			// Display status code
			if (log != null && log.isDebug())
				log.logDebug(MDPersonator.getLogString("ResponseCode", String.valueOf(statusCode)));
			if (statusCode != -1) {
				if (Const.isEmpty(webEncoding)) {
					inputStreamReader = new InputStreamReader(method.getResponseBodyAsStream());
					if ((log != null) && log.isDebug()) {
						log.logDebug(MDPersonator.getLogString("Encoding", inputStreamReader.getEncoding()));
					}
				} else {
				...
				if ((log != null) && log.isDebug()) {
					log.logDebug(MDPersonator.getLogString("ResponseBody", response.toString()));
				}
			}
	...
	public int checkProxy(final URL url, final String request, final StringBuffer response) {
		...
		try {
			...
			MDPersonatorData.webInitMsg = e.getMessage();
		}
		proxyCheck = false;
	...
	public void downloadFile(URL link, String file) throws KettleException {
		...
		try {
			...
			if (statusCode != 200)
				throw new KettleException(MDPersonator.getErrorString("StatusCodeNot200", "" + statusCode));

			// Write the response to the file
			writer.append(response);
		} catch (IOException e) {
			throw new KettleException(MDPersonator.getErrorString("BadReportFile", file, e.getMessage()));
		} finally {
			if (writer != null)
				try {
					writer.close();
